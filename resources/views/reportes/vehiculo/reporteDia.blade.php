                            ...
                            @foreach ($listaVehiculos as $vehiculo)
                                <div class="card mb-3">
                                    <div class="card-header bg-light">
                                        ...
                                        <span @php if($vehiculo->estado == 'Activo'){ echo'class="btn btn-success"'; }else{ echo'class="btn btn-danger"';} @endphp>{{ $vehiculo->estado }}</span>
                                    </div>
                                    <div class="card-body">
                                        ...
                                        @foreach ($horarios as $horario)
                                            @foreach ($listaActividades as $actividad)
                                                @if ($horario->id == $actividad->horario_id && $vehiculo->id == $actividad->vehiculo_id)
                                                    <p class="card-text">
                                                        <span class="btn btn-secondary">{{ $horario->hora }}</span> <span class="btn btn-primary">{{ $actividad->actividad }}</span> <span class="btn btn-light">{{ $actividad->nombre }}</span></p>
                                                @endif
                                            @endforeach
                                        @endforeach
                                    </div>
